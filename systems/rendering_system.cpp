...
{
	if(layer != entity_system().entity(entity)->visuals()->layer())
	{
		remove_id(entity);
		entity_system().entity(entity)->visuals()->set_layer(layer);
		add_id(entity);
	}
}
...
void RenderingSystem::render_entities(Time time_diff, bool paused, SDL_Renderer* renderer) const
{
    ...
    if(entity_system().entity(0) == nullptr)
    ...
    for(auto layer = 0; layer < Visuals::NUM_OF_LAYERS; ++layer)
    {
		for(auto it = entities[layer].begin(); it != entities[layer].end(); ++it)
		{
			Visuals* visuals = entity_system().entity(*it)->visuals();
			...
			Spritesheet* spritesheet = resource_system().spritesheet(visuals->spritesheet_id());
			...
			{
				SDL_Rect hitbox
					{ int(entity_system().entity(*it)->position()->x() - screen_zone_position->x())
					, int(globals().resolution_y-entity_system().entity(*it)->position()->h() - entity_system().entity(*it)->position()->y() + screen_zone_position->y())
					, int(entity_system().entity(*it)->position()->w())
					, int(entity_system().entity(*it)->position()->h())
					};
